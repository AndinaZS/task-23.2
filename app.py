...
def filter_func(data, param: str) -> filter:
    return filter(lambda x: param in x, data)

def map_func(data, param: str) -> map:
    return map(lambda x: f'{x[int(param)]}\n', [el.split() for el in data])

def sorted_func(data, param: str) -> List:
    return sorted(data, reverse=True if param == 'desc' else False)

def limit(data, param: str) -> List:
    return [next(data) for _ in range(int(param))]

def unique(data, param: str ='') -> Set:
    return set(data)

def regex(data, param: str) -> Generator:
    for d in data:
